...
class NETlib():
    ...
    def define_instrument(self, opts):
        ...
        if 'Tc' not in bolo.keys():
            bolo['Tc']  = 2 *  bolo['T0']  # bath temperature
        if 'sf' not in bolo.keys():
            if band['v_cen'] < 60e9:
                ...
                bolo['sf'] = 3.0 ;
        if 'beta' not in bolo.keys():
            bolo['beta'] = 2.0 # conductance scales at T^beta
        if 'Rtes' not in bolo.keys():
            bolo['Rtes'] = 0.05 #  Ohm
       ...
       ## Define layers
       # each layer has a physical temperature (T), an emissivity (eps),
       # and transmissivity (Tx)
       ...
       # optical efficiency is Tx through the whole instrument.
        ...
        if 's' in  opts.keys():
            s = opts['s']
            if (self.verbose): print('###   User-defined opts for instrument layers')
        else:
            if (self.verbose): print('###   Default instrument layers')
            s={}
    ...
    def  calc_photon_noise(self,atm,band,s):
        ...
        for i in range(Nl):
            ...
            if(self.verbose):
                print('%12s Tx:%2.2f, cumulTx to det: %2.3f, Power:%2.3f [pW], Trj:%6.3f [Krj]'
        ...
        if self.verbose:
            print('                                       Total Power:%2.3f [pW], Trj:%6.3f [Krj]'
                  %(Q.tot *1e12, Q.Trjtot))
...
####
    def readAmResults(self,outfn):
        ...
        try:
            ...
            inErrFile = open(outfn.replace('.out','.err'),'r')
